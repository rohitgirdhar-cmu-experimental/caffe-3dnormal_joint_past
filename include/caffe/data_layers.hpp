...
template <typename Dtype>
class JointImageDataLayer : public Layer<Dtype> {
  // The function used to perform prefetching.
  friend void* JointImageDataLayerPrefetch<Dtype>(void* layer_pointer);

 public:
  explicit JointImageDataLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual ~JointImageDataLayer();
  virtual void SetUp(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top);
 ...
 protected:
  ...
  pthread_t thread_;
  shared_ptr<Blob<Dtype> > prefetch_data_;
  shared_ptr<Blob<Dtype> > prefetch_data_2_;
  shared_ptr<Blob<Dtype> > prefetch_label_;
  Blob<Dtype> data_mean_;
