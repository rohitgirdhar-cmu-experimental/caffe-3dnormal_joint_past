...
template <typename Dtype>
void Solver<Dtype>::MultiSnapshot() {
  NetParameter net_param;
  // For intermediate results, we will also dump the gradient values.
  net_->ToProto(&net_param, param_.snapshot_diff());
  string filename(param_.snapshot_prefix());
  ...
  vs.push_back(param_.model_1());
  vs.push_back(param_.model_2());
  vs.push_back(param_.model_3());
    ...
    for(int i = 0; i < 3; i ++)
  	{
		...
		ReadProtoFromTextFile(vs[i], &trained_net_param);
  		//ReadProtoFromBinaryFile(vs[i], &trained_net_param);
  		Net<float> caffe_net_now(trained_net_param);
  		caffe_net_now.CopyTrainedLayersFrom(net_param);
  		NetParameter save_net_param;
  		caffe_net_now.ToProto(&save_net_param);
  		string mt;
  		mt = "0"; mt[0] += i;
  		string nows = filename + "_" + mt + iter_str_buffer;
  		WriteProtoToBinaryFile(save_net_param, nows.c_str());
  	}
