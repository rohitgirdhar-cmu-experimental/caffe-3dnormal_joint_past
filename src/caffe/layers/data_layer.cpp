...
void* DataLayerPrefetch(void* layer_pointer)
{
	...
	//	sprintf(ss2,"/home/dragon123/cnncode/showimg/%d_gt.jpg",item_id);
		...
		if (crop_size)
		{
			...
			if (layer->phase_ == Caffe::TRAIN && layer->layer_param_.rand_crop())
			{
		...
		/*		int h = label_i % 50;
				int w = label_i / 50;
				img.at<cv::Vec3b>(h, w)[0] = (uchar)(datum.label(label_i) * 255);
				img.at<cv::Vec3b>(h, w)[1] = (uchar)(datum.label(label_i) * 255);
				img.at<cv::Vec3b>(h, w)[2] = (uchar)(datum.label(label_i) * 255);*/
			}
...
void DataLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if (output_labels_)
  {
   ...
   // prefetch_label_.reset( new Blob<Dtype>(this->layer_param_.data_param().batch_size(), 1, 1, 1) );
	  ...
	  prefetch_label_.reset( new Blob<Dtype>(this->layer_param_.data_param().batch_size(), datum.label_size(), 1, 1) );
  }
  // datum size
  ...
  if (this->layer_param_.data_param().has_mean_file()) {
    ...
    /*FILE * file = fopen("/home/dragon123/3dnormal_joint_cnncode/videomean.txt", "w");
	...
	//for(int h = 0; h < datum_height_; h ++) 
	  //  for(int w =0 ; w < datum_width_; w ++)
	//	data_mean_.data_at(0,c,h,w) = 128;
