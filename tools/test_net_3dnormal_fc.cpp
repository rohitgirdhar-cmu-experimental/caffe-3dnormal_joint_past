...
int main(int argc, char** argv)
{
	...
	float * st = new float[LABEL_LEN * LABEL_SIZE * LABEL_SIZE];
		...
		for (int i = 0; i < bsize && counts < data_counts; i++, counts++)
		{
			...
			//for(int c = 0; c < LABEL_LEN; c++)
			{
				for(int w = 0; w < LABEL_SIZE; w ++)
					for(int h = 0; h < LABEL_SIZE; h ++)
					{
						float tnum1 = (float)(bboxs->data_at(i, h * LABEL_SIZE + w, 0, 0));
						float tnum2 = (float)(bboxs->data_at(i, LABEL_SIZE * LABEL_SIZE + h * LABEL_SIZE + w, 0, 0));
						float tnum3 = (float)(bboxs->data_at(i, 2 * LABEL_SIZE * LABEL_SIZE + h * LABEL_SIZE + w, 0, 0));
						float z = tnum1 * tnum1 + tnum2 * tnum2 + tnum3 * tnum3;
						z = sqrt(z);
						st[w * LABEL_SIZE + h] = tnum1 / z;
						st[LABEL_SIZE * LABEL_SIZE + w * LABEL_SIZE + h] = tnum2 / z;
						st[2 * LABEL_SIZE * LABEL_SIZE + w * LABEL_SIZE + h] = tnum3 / z;
			...
			for(int c = 0; c < LABEL_LEN; c++)
			{
				for(int w = 0; w < LABEL_SIZE; w ++)
					for(int h = 0; h < LABEL_SIZE; h ++)
					{
						float tnum = st[c * LABEL_SIZE * LABEL_SIZE + w * LABEL_SIZE + h] ;
						fprintf(resultfile, "%f ", tnum);
			//			imgOut.at<cv::Vec3b>(h, w)[2 - c] = (uchar)((tnum) * 128 + 128);
					}
			}
